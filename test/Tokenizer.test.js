...
describe('Tokenizer tests', () => {
  describe('Text grammar', () => {
    ...
    it("TC2 input 'a aa' sequence [>] is of token type WORD and value is 'aa'", () => {
      expect(new Tokenizer(textGrammar, 'a aa').getNextToken().Token).toBe(
        'WORD'
      )
    ...
    it("TC4 input 'a.b' sequence [>>] is of token type WORD and value is 'b'", () => {
      expect(new Tokenizer(textGrammar, 'a.b').getNextToken(2).Token).toBe(
        'WORD'
      )
    ...
    it("TC5 input 'aa. b' sequence [>>] is of token type WORD and value is 'b'", () => {
      expect(new Tokenizer(textGrammar, 'aa. b').getNextToken(2).Token).toBe(
        'WORD'
      )
      expect(new Tokenizer(textGrammar, 'aa. b').getNextToken(2).Value).toBe(
        'b'
      )
    })
    ...
    it("TC10 input 'a' sequence [<] throws IndexException", () => {
      expect(() => new Tokenizer(textGrammar, 'a').getPreviousToken()).toThrow(
        IndexException
      )
    })
  ...
  describe('Arithmetic grammar', () => {
    ...
    it("TC14 input '3 + 54 * 4' sequence [>>>] is of token type MUL and value is '*'", () => {
      expect(
        new Tokenizer(arithmeticGrammar, '3 + 54 * 4').getNextToken(3).Token
      ).toBe('MUL')
      expect(
        new Tokenizer(arithmeticGrammar, '3 + 54 * 4').getNextToken(3).Value
      ).toBe('*')
    })
    it("TC15 input '3+5 # 4' sequence [>>>] throws InvalidTokenException", () => {
      expect(() =>
        new Tokenizer(arithmeticGrammar, '3+5 # 4').getNextToken(3)
    ...
    it("TC19 input ')(' sequence [>] is of token type OPENING and value is '('", () => {
      expect(new Tokenizer(arithmeticGrammar, ')(').getNextToken().Token).toBe(
        'OPENING'
      )
    ...
    it("TC20 input '3(5-2)' sequence [>>>>>] is of token type CLOSING and value is ')'", () => {
      expect(
        new Tokenizer(arithmeticGrammar, '3(5-2)').getNextToken(5).Token
      ).toBe('CLOSING')
      expect(
        new Tokenizer(arithmeticGrammar, '3(5-2)').getNextToken(5).Value
      ).toBe(')')
    })
    it("TC21 input '3 = 4 - 1' sequence [>] is of token type EQUAL and value is '='", () => {
      expect(
        new Tokenizer(arithmeticGrammar, '3 = 4 - 1').getNextToken().Token
      ).toBe('EQUAL')
      expect(
        new Tokenizer(arithmeticGrammar, '3 = 4 - 1').getNextToken().Value
      ).toBe('=')
    })
  })
